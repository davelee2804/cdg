...
xp = np.loadtxt( 'output/pgrid_' + '%.3u'%nx + '_2.x.txt' )
yp = np.loadtxt( 'output/pgrid_' + '%.3u'%nx + '_2.y.txt' )
Xp,Yp = np.meshgrid( xp, yp )
diff2 = np.loadtxt( 'output/diff_' + '%.3u'%nx + '.txt' )
plt.contourf( Xp, Yp, diff2.reshape( ( len(yp), len(xp) ) ), 100 )
plt.colorbar()
plt.savefig( 'diff2' + '.' + '%.3u'%nx + '.png' )
plt.figure()
